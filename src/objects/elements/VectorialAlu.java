  ...
  public void signalChanged() {
    if (control.size() == 1 && input.size() == 16 && output.size() == 8) {
      if (control.get(0).getState() == 1) { //ADDV
        for (int i = 0; i < 8; i++) {
          int value = input.get(i).getState();
          int addition = input.get(i + 8).getState();
          while (addition > 0) {
            addition--;
            value++;
            if (value > 255) {
          ...
          output.get(i).setState(value);
        }
      }
      if (control.get(0).getState() == 2) { //SUBV
        for (int i = 0; i < 8; i++) {
          int value = input.get(i).getState();
          int substraction = input.get(i + 8).getState();
      ...
      if (control.get(0).getState() == 3) { //XOR
        for (int i = 0; i < 8; i++) {
      ...
      if (control.get(0).getState() == 7) { //SRLV
        for (int i = 0; i < 8; i++) {
      ...
      if (control.get(0).getState() == 8) { //SLLV
        for (int i = 0; i < 8; i++) {
          String bits = Integer.toBinaryString(input.get(i).getState());
          while (bits.length() < 8) {
            bits = "0" + bits;
